...
session_start();
if(!empty($_POST)){
	if(isset($_POST["productos_id"]) && isset($_POST["q"])){
		// si es el primer producto simplemente lo agregamos
		if(empty($_SESSION["cart"])){
			$_SESSION["cart"]=array( array("productos_id"=>$_POST["productos_id"],"q"=> $_POST["q"]));
		}else{
			...
			foreach ($cart as $c) {
				...
				if($c["productos_id"]==$_POST["productos_id"]){
					$repeated=true;
					break;
				}
			}
			...
			if($repeated){
				...
				array_push($cart, array("productos_id"=>$_POST["productos_id"],"q"=> $_POST["q"]));
				$_SESSION["cart"] = $cart;
			}
		}
